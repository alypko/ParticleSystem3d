...
void ParticleData::generate(size_t maxSize)
{
    m_count = maxSize;
    m_countAlive = 0;
    m_pos.reset(new ofVec3f[maxSize]);
    m_col.reset(new glm::vec4[maxSize]);
    m_startCol.reset(new glm::vec4[maxSize]);
    m_endCol.reset(new glm::vec4[maxSize]);
    m_vel.reset(new ofVec3f[maxSize]);
    m_acc.reset(new ofVec3f[maxSize]);
    m_time.reset(new ofVec3f[maxSize]);
...
void ParticleSystem::update(double dt)
{
    ...
    {
        m_particles.m_acc[i] = ofVec3f(0.0f);
    }
